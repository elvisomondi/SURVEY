...
class InstallerController extends CController {
    ...
    */
    private function stepWelcome()
    {
        ...
        $aData['descp'] = gT('Welcome to the Utafiti installation wizard. This wizard will guide you through the installation, database setup and initial configuration of Utafiti.');
    ...
    */
    private function stepPreInstallationCheck()
    {
        ...
        $aData['descp'] = gT('Pre-installation check for Utafiti ').Yii::app()->getConfig('versionnumber');
    ...
    */
    private function stepDatabaseConfiguration()
    {
        ...
        $aData['descp'] = gT('Please enter the database settings you want to use for Utafiti:');
            ...
            if($oModel->validate()) {
                ...
                if ($bDBConnectionWorks)
                {
                    ...
                    if ($bDBExists && !$bTablesDoNotExist)
                    {
                        ...
                        $aData['noticeMessage'] = gT('The database exists and contains Utafiti tables.');
                    ...
                    if (in_array($oModel->dbtype, array('mysql', 'mysqli'))) {
                        ...
                        {
                            die("<br />Error: You need at least MySQL version 4.1 to run Utafiti. Your version: ".$sMySQLVersion);
                        }
                    ...
                    elseif (!$bDBExistsButEmpty)
                    {
                        ...
                        if ($output== '') {$aValues['adminoutput'].='<br />'.gT('Utafiti database is up to date. No action needed');}
        ...
        if(!is_null(Yii::app()->request->getPost('InstallerConfigForm')))
        {
            ...
            if($model->validate()) {
                ...
                if ($this->connection->getActive() == true) {
                    ...
                    $aData['descp'] = gT("Utafiti has been installed successfully.");
    ...
    function _writeConfigFile()
    {
        ...
        if (Yii::app()->session['databaseexist'] && Yii::app()->session['tablesexist'])
        {
            ...
            */
            ...
            ."\t"     . "// on your webspace.". "\n"
            ."\t"     . "// Utafiti developers: Set this to 2 to additionally display STRICT PHP error messages and get full access to standard templates". "\n"
            ."\t\t"   . "'debug'=>0,"                               . "\n"
            ."\t\t"   . "'debugsql'=>0, // Set this to 1 to enanble sql logging, only active when debug = 2" . "\n"
            ."\t\t"   . "// Update default Utafiti config here"  . "\n"
            ."\t"     . ")"                                         . "\n"
            . ");"                                        . "\n"
    ...
    */
    private function dbTest($aDbConfig = array(), $aData = array())
    {
        ...
        {
            if (!empty($aData['model'])) {
                $aData['model']->addError('dblocation', sprintf(gT('The database does not meet the minimum MySQL/MariaDB server version requirement for Utafiti (%s). Found version: %s'),$sMinimumMySQLVersion, $testPdo->getAttribute(constant("PDO::ATTR_SERVER_VERSION"))));
                $this->render('/installer/dbconfig_view', $aData);
                Yii::app()->end();
            }
        }
